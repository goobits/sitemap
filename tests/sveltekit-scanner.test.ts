...
describe('scanSvelteKitRoutes', () => {
	...
	it('applies exclude predicate', () => {
		const entries = scanSvelteKitRoutes(
			...
			{
				exclude: (path, raw) =>
					raw.includes('(protected)') || raw.includes('local-only') || path === '/sitemap'
			}
		)
		const paths = entries.map((e) => e.path).sort()
	...
	it('marks hasServerLoad from serverGlob', () => {
		const entries = scanSvelteKitRoutes(
			mockGlob(['/src/routes/+page.svelte', '/src/routes/about/+page.svelte']),
			{
				serverGlob: mockGlob([
	...
	it('applies custom lastModified resolver', () => {
		const entries = scanSvelteKitRoutes(mockGlob(['/src/routes/about/+page.svelte']), {
			lastModified: () => TS
		})
